...
Subsequent information displayed to the terminal shows all packets that have been copied over to the primary network interface for filtering. Targets that appear are ALL layer layer 3 (Network) packets copied and sent over to the NIC defined by "device string" within `detectpacket.go`.
* `detectpacket.go` captures ALL layer 3 (Network) traffic sent to NIC.
